...
describe("useAssetsStore", () => {
  ...
  it("loads list by type", async () => {
    vi.spyOn(API, "listAssets" as any).mockResolvedValue({ items: [{ id: "1", type: "scene", name: "A", description: "", voice_style: "", image_path: null, audio_path: null, source_project: null, updated_at: null }] });
  ...
  it("removes asset locally after delete", async () => {
    useAssetsStore.setState({ byType: { character: [], scene: [{ id: "1", type: "scene", name: "A", description: "", voice_style: "", image_path: null, audio_path: null, source_project: null, updated_at: null }], prop: [] } });
