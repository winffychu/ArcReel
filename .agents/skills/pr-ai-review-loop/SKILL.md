...
3. 循环期间的所有 actionable 评论均已实施修复或记录 pushback。终核时对两家 AI reviewer 各跑一次 `query.sh unacked <bot[bot]>` 兜底——返回的每条须已修复或有在案 pushback,防 `is_new` / `is_ack` 误判漏标
...
子命令:`details <id>...`(按 id 批量取全文)/ `gemini-latest-body` / `quality-all`(终核)/ `history`(主题重复)/ `unacked <bot[bot]>`(fix-up 顺延核对未解决评论;bot 名带 `[bot]` 后缀,如 `coderabbitai[bot]`)/ `index`(重印上轮全量索引)。查询异常一律以 `QUERY_ERROR` 响亮失败——空结果因此可以放心当作确无数据。
